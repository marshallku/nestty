...
- [x] **Bounded writer_tx (5b R2 INFO):** daemon-side `GuiClient.writer_tx` changed from `mpsc::Sender` (unbounded) to `mpsc::SyncSender(512)`. Closes the "wedged GUI socket writer lets the event forwarder accumulate unbounded memory" hazard codex flagged on the 5b review. 512-line buffer is plenty for normal bursts; when full, the forwarder blocks on `send`. Recovery: heartbeat-miss → `fail_all_pending` → `shutdown(SHUT_RDWR)` → writer thread exits → forwarder unblocks with `Disconnected` and exits. All 17 gui_registry tests still pass (test channels updated to `sync_channel(64)`).
